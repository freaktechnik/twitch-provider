...
const replaceEmote = (msg, code, srcset) => {
    ...
    let count = 0;
    const img = "<img alt='" + code + "' title='" + code + "' srcset='" + srcset + "' style='vertical-align: middle;'>";
...
const TwitchChannel = Class({
    ...
    _addChatters(chatters, skipCheck = false) {
        //TODO optionally disable adding all chatters (good for low-spec devices and TB)
        ...
        const workOnQueue = (i, category, interval) => {
            ...
            while(i < goalIndex) {
                ...
                p = this.getParticipant(chatters[category][i], !skipCheck, skipCheck);
                ...
                if(skipCheck && !hadParticipant && (p.prefs.show_all_chatters || category !== "viewers"))
                    participants.push(p);
    ...
    prepareForDisplaying: function(msg) {
        ...
        //TODO avoid this breaking incoming emotes
