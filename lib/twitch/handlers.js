...
const ARGUMENT_TAGS = [ "target_channel", "number", "target_user", "ban_duration", "command" ];
...
function getArguments(message) {
    ...
    for(let arg of ARGUMENT_TAGS) {
        if(message.tags.has("msg-param-"+arg))
            args.push(message.tags.get("msg-param-"+arg));
    }
...
let ctcpHandlers = {
    ...
    commands: {
       "ACTION": function(message) {
            const params = {
                incoming: true,
                color: getColor(message.tags.get("color")),
                emotes: message.tags.has("emotes") ? message.tags.get("emotes").split("/") : []
            };
            const displayName = message.tags.get("display-name") || message.origin;
    ...
    commands: {
        "NOTICE": function(message) {
            if(message.params[0].charAt(0) == "#") {
                ...
                else if(!SUPPORTED_MSG_IDS.includes(message.tags.get("msg-id"))) {
                    //TODO get arguments
                    conversation.writeMessage(message.origin, _(message.tags.get("msg-id"), ...getArguments(message)), {system: true});
                }
        ...
        "CLEARCHAT": function(message) {
            if(message.params.length > 1) { // messages of a user have been cleared
                ...
                if(msg.tags.has("ban-reason") {
                    ...
                    if(msg.tags.has("ban-duration"))
                        msgtext = _("timeout", participant.name, message.tags.get("ban-duration"));
                    else
                        msgtext = _("clear_user", participant.name);
                }
                conversation.writeMessage(message.origin, msgtext, {system: true});
        ...
        "PRIVMSG": function(message) {
            ...
            let displayName = message.tags.get("display-name") || message.tags.get("login") || message.origin;
            ...
            if(message.origin == "twitchnotify" || message.origin == "jtv") { // subscription notifications and such shit
                params.system = true;
                 ...
                 params.emotes = message.tags.has("emotes") ? message.tags.get("emotes").split("/") : [];
        ...
        "USERNOTICE": function(message) {
            const conversation = this.getConversation(message.params[0]);
            const displayName = message.tags.get("display-name") || message.tags.get("login") || message.origin;

            conversation.writeMessage(message.origin, _("resub", displayName, message.tags.get("msg-param-months")), {system: true});
