...
const TwitchChannel = Class({
        ...
        for(let category in chatters) {
            if(category !== "viewers" || skipCheck) {
                chatters[category].forEach((user) => {
                    let p = this.getParticipant(user, !skipCheck);
    ...
    broadcasterLanguage: "",
    _hasParticipant(who) {
        if(this._participants.get(who) || BUILTINS.includes(who))
            return true;

        const unnormalized = this._findUnnormalizedParticipant(who);

        return unnormalized !== null;
    },
