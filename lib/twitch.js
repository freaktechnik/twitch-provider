...
const replaceEmoticon = (msg, code, id) => msg.replace(new RegExp("(^|\\s)"+escapeForRegExp(code)+"(?=\\s|$)", "g"), "$1<img alt='"+code+"' src='http://static-cdn.jtvnw.net/emoticons/v1/"+id+"/1.0' style='vertical-align: middle;'>")
    ...
    prepareForDisplaying: function(msg) {
        ...
        if(meta && meta.emotes && !meta.system) {
            meta.emotes.forEach((emote) => {
                ...
                msg.displayMessage = replaceEmoticon(msg.displayMessage, emoteSequence, id);
            });
        }
