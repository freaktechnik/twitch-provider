    ...
    prepareForDisplaying: function(msg) {
        ...
        if(meta && meta.emotes && !meta.system) {
            meta.emotes.forEach((emote) => {
                ...
                msg.displayMessage = msg.displayMessage.replace(new RegExp("(^|\\s)"+escapeForRegExp(emoteSequence)+"(?=\\s|$)", "g"), "$1<img alt='"+emoteSequence+"' src='http://static-cdn.jtvnw.net/emoticons/v1/"+id+"/1.0' style='vertical-align: middle;'>");
            });
        }
