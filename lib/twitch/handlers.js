...
function getBadges(msg, account) {
    ...
    ret.forEach((badge) => {
        if(!account._badges.includes(badge))
...
exports.handlers = {
    ...
    commands: {
        ...
        "USERSTATE": function(message) {
            ...
            //TODO badges for this the self participant
            ...
            if(message.origin == "twitchnotify" || message.origin == "jtv") { // subscription notifications and such shit
                params.system = true;
                 ...
                 if(message.params[1].substr(1) === displayName.toLowerCase() && !participant._badges.includes("owner"))
                    participant._badges.push("owner");
                participant._conv.notifyObservers(participant, "chat-buddy-update");
            }
