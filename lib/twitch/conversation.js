...
//TODO get participants list on reconnect
...
const TwitchChannel = Class({
    ...
    _addChatters(chatters, skipCheck = false) {
        ...
        if(this._isFilled) {
            ...
            this._participants.forEach((p, n) => {
                if(p.name === this._account.name)
                    remove = false;
                else if(p.hasBadge("admin"))
                    remove = !chatters.admins.includes(n);
                else if(p.hasBadge("global_mod"))
                    remove = !chatters.global_mods.includes(n);
                else if(p.hasBadge("staff"))
                    remove = !chatters.staff.includes(n);
                else if(p.hasBadge("mod"+BADGE_CHANNEL_SPLITTER+this.name))
                    remove = !chatters.moderators.includes(n);
                else
                    remove = !chatters.viewers.includes(n);
    ...
    prepareForDisplaying: function(msg) {
        ...
        if(msg.incoming && !msg.system &&
          (
            p.prefs.ignore_bots && meta.hasBadge("bot") ||
            (
              p.prefs.ignored_messages.length > 0 &&
              p.prefs.ignored_messages.split(",").includes(msg.message)
            )
          )
        )
            return;
        ...
        if(msg.outgoing) {
            ...
            if(meta && "_emotes" in meta && meta._emotes.length) {
