...
function getBadges(msg, account) {
    const badges = msg.tags.get('badges').split(",");
    for(const badge of badges) {
        const [ name, version ] = badge.split('/');
        account.addBadge(getBadge(name, msg.params[0]), version);
    }
    //TODO remove badges.
}
...
const handlers = {
    ...
    commands: {
        ...
        USERNOTICE(message) {
            ...
            switch(message.tags.get('msg-id')) {
                ...
                case 'crate':
                    ...
                    break;
                case 'submysterygift':
                    msg = _('submysterygift', displayName, message.tags.get('msg-param-mass-gift-count'), _(`tier_${message.tags.get('msg-param-sub-plan')}`), conversation._twitchName, message.tags.get('msg-param-sender-count'));
                    break;
                ...
                case 'anonsubgift': {
                    const giftee = message.tags.get('msg-param-recipient-display-name') || message.tags.get('msg-param-recipient-user-name');
                    msg = _('anonsubgift', message.tags.get('msg-param-sub-plan-name'), giftee);
                    //TODO set giftee to sub -> msg-param-recipient-user-id tag
                    break;
                }
                case 'anonsubmysterygift':
                    msg = _('anonsubmysterygift', message.tags.get('msg-param-mass-gift-count'), _(`tier_${message.tags.get('msg-param-sub-plan')}`), conversation._twitchName);
                    break;
                case 'anongiftpaidupgrade':
                    msg = _('anongiftpaidupgrade', message.tags.get('msg-param-sender-name') || message.tags.get('msg-param-sender-login'), message.tags.get('msg-param-promo-gift-total'), message.tags.get('msg-param-promo-name'));
                case 'ritual':
        ...
        CLEARMSG() {
            // currently no client side support to handle this fully.
            const conversation = this.getConversation(message.params[0]);
            conversation.writeMessage(message.origin, _("message_delete_success"), { system: true, tags: message.tags });
            return true;
        }
    }
};
...
const tagHandlers = {
    ...
    isEnabled: checkAccount,
    commands: {
        //TODO display-name should set display name on message sender. Same for user-id and badges.
        color(tagMsg) {
        ...
        'tmi-sent-ts'(tagMsg) {
            tagMsg.message.time = Math.floor(parseInt(tagMsg.tagValue, 10) / 1000);
        }
    }
};
