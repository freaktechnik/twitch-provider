...
const TwitchAccount = Class({
    ...
    requestBuddyInfo: function(nick) {
        ...
        let info = nsSimpleEnumerator(badges.map((badge) => {
            if(badge.includes(BADGE_CHANNEL_SPLITTER)) {
                let info = badge.split(BADGE_CHANNEL_SPLITTER);
                return TooltipInfo(_("badge_"+info[0], info[1]), "");
            }
            else {
                return TooltipInfo(_("badge_"+badge), "");
            }
        }));
        emit("user-info-received", {
            subject: info,
            data: nick
        });
    },
