...
const TwitchChannel = Class({
    ...
    initialize: function(account, convName, nick) {
        Channel.call(this, account, this.normalize(convName), nick);
    ...
    _addChatters(chatters, skipCheck = false) {
        ...
        let remove = false;
        for(let p of this._participants.keys()) {
            if(p.hasBadge("admin"))
                remove = !chatters.admins.includes(p.name);
            else if(p.hasBadge("global_mod"))
                remove = !chatters.global_mods.includes(p.name);
            else if(p.hasBadge("staff"))
                remove = !chatters.staff.includes(p.name);
            else if(p.hasBadge("mod"+BADGE_CHANNEL_SPLITTER+this.name))
                remove = !chatters.moderators.includes(p.name);
            else
                remove = !chatters.viewers.includes(p.name);
