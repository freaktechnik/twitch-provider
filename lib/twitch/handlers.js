...
const SUPPORTED_MSG_IDS = [ "subs_on", "subs_off", "slow_on", "slow_off", "r9k_on", "r9k_off", "host_on", "host_off", "timeout_success", "ban_success", "cmds_available", "emote_only_on", "emote_only_off" ];
    ...
    commands: {
        ...
        "CLEARCHAT": function(message) {
            if(message.params.length > 1) { // messages of a user have been cleared
                ...
                const conversation = this.getConversation(message.params[0]);
                conversation.writeMessage(message.origin, _("chat_cleared"), {system: true});
            }
        },
        ...
        "PRIVMSG": function(message) {
            ...
            if(message.tags.get("historical") == "1") {
                params.delayed = true;
                params.time = Math.floor(parseInt(message.tags.get("tmi-sent-ts"), 10) / 1000);
            }
