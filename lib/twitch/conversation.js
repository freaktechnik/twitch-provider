...
const replaceEmoticon = (msg, code, id) => {
    ...
    return replaceEmote(msg, code, srcset, '(max-width: 28px)');
};
...
const replaceEmote = (msg, code, srcset, sizes) => {
    // Replace emotes only if they are free standing
    let pattern = new RegExp("(^|\\s)"+code+"(?=\\s|$)", "g");
    let count = 0;
    let newMsg = msg.replace(pattern, (math, p1) => {
        ++count;
        return p1 + "<img alt='" + code + "' srcset='" + srcset + "' sizes='" + sizes + "' style='vertical-align: middle;'>";
    });
    return [ newMsg, count ];
};
...
const TwitchChannel = Class({
    ...
    initialize: function(account, convName, nick) {
        ...
        twitchAPI.getViewers(this._name.slice(1)).then((viewerCount) => {
    ...
    _findUnnormalizedParticipant: function(w) {
        if(w !== this.normalizeNick(w))
            return;

        return [...this._participants.keys()].find((n) => this.normalizeNick(n) == w);
    },
    _viewerCount: 0,
    ...
    broadcasterLanguage: "",
    getParticipant: function(who, notifyObservers = false) {
        ...
        // Find if there is an unnormalized version of the username in who.
        let unnormalized = this._findUnnormalizedParticipant(who);

        if(unnormalized)
            return this._participants.get(unnormalized);

        if(this._participants.has(this.normalizeNick(who)))
            return this._participants.get(this.normalizeNick(who));
    ...
    removeParticipant: function(nick) {
        if(!this._participants.has(nick)) {
            let unnormalized = this._findUnnormalizedParticipant(nick);
        ...
        Channel.prototype.removeParticipant.call(this, nick);
    },
    writeMessage: function(who, msg, obj) {
        if(!obj.system) {
            let normalized = this.normalizeNick(who);
            if(normalized !== who && this._participants.has(normalized)) {
            ...
            if(who !== this._account._nickname && this._isFilled && !participant.op) {
    ...
    prepareForDisplaying: function(msg) {
        //TODO replace username highlights with correct-case versions (performance?)
        let meta;
        if(!msg.system) {
        ...
        const doReplace = (emote) => {
            [ msg.displayMessage ] = replaceEmote(msg.displayMessage, escapeForRegExp(emote.code), emote.srcset, emote.sizes);
        };

        //TODO channel specific 3rd party emotes
        if(bttv.enabled) {
    ...
    set topic(newTopic) {
        if(this.topicSettable) {
            twitchAPI.setTitle(this.title.slice(1), this._account._nickname, newTopic).then(() => {
    ...
    setMode() { /* no mode changes for channels */ }
});
