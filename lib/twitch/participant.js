...
const Participant = Class({
    ...
    get founder() {
        return this._badges.includes("owner");
    },
    get voiced() {
        return this._badges.includes("sub"+BADGE_CHANNEL_SPLITTER+this._conv.name);
    },
    get op() {
        return this._op || this.founder || this._badges.some((badge) => GLOBAL_OPS.includes(badge)) || this._badges.includes("mod"+BADGE_CHANNEL_SPLITTER+this._conv.name);
    }
});
