...
const replaceEmote = (msg, code, srcset, sizes) => {
    ...
    let count = 0;
    let img = "<img alt='" + code + "' srcset='" + srcset + "' sizes='" + sizes + "' style='vertical-align: middle;'>";
...
const TwitchChannel = Class({
    ...
    initialize: function(account, convName, nick) {
        ...
        twitchAPI.canSetTitle(account._nickname, this._twitchName).then((can) => {
        ...
        twitchAPI.getViewers(this._twitchName).then((viewerCount) => {
        ...
        }, () => console.log("No bttv emotes for", this._twitchName));
        frankerfacez.getChannelEmotes(this._twitchName).then((emotes) => {
            this._frankerzEmotes = emotes;
        }, () => console.log("No frankerfacez emotes for", this._twitchName));
    },
